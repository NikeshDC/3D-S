...
class Surface:
    ...
    def __init__(self,v1: Vertex,v2: Vertex,v3: Vertex, material = None):
        ...
        self.center = Vertex((v1.x+v2.x+v3.x)/3, (v1.y+v2.y+v3.y)/3, (v1.z+v2.z+v3.z)/3)
    ...
    def addVertex(self,v):
        ...
        n = len(self.vertices)
        self.center = Vertex((self.center.x * n+v.x)/(n+1), (self.center.y * n+v.y)/(n+1), (self.center.z * n+v.z)/(n+1))
...
##            vertex.vy = vertex.vy * camera.Zvp / vertex.vz
    ...
    def scale_center(self, sx,sy,sz, camera):
        ...
        self.scaleFP(self.center.x, self.center.y, self.center.z, sx, sy, sz, camera)
...
class Model:
    ...
    def __init__(self, wireframeColor = (241,165,5), material = Material() , shading = Shading.FLAT):
        self.surfaces = []
        self.vertices = []
        self.edges = []
    ...
    def display_wireframe(self, screen):
        for edge in self.edges:
            pygame.draw.aaline(screen, self.wireframeColor, topixel(edge.start.vx, edge.start.vy), topixel(edge.end.vx, edge.end.vy))
    ...
    def scale_surfaces_center(self, surfaces, scale, camera):
        ...
        center_y /= vn
        center_z /= vn
        self.scale_surfacesFP(surfaces, scale,(center_x, center_y, center_z), camera)
...
class Light:
    ...
    def __init__(self, pos, intensity = 1, edgecolor = (255,255,255), selected_color = (255,255,0)):
        self.x = pos[0]
        self.y = pos[1]
        self.z = pos[2]
        self.i = intensity
        norm_abs = math.sqrt(self.x* self.x + self.y* self.y + self.z* self.z)
        self.normvector = (self.x/norm_abs, self.y/norm_abs, self.z/norm_abs)
    ...
    def display(self, screen):
        pygame.draw.aaline(screen, self.edgecolor, topixel(self.edges[0].start.vx, self.edges[0].start.vy), topixel(self.edges[0].end.vx, self.edges[0].end.vy))
        if self.selected:
            self.rect = pygame.draw.circle(screen, self.selected_color, topixel(self.edges[0].start.vx, self.edges[0].start.vy), self.radius)
        else:
            self.rect = pygame.draw.circle(screen, self.edgecolor, topixel(self.edges[0].start.vx, self.edges[0].start.vy), self.radius)
    ...
    def translate(self,tx,ty,tz, camera):
        ...
        camera.setViewingValues(self.vertices)
        self.x, self.y, self.z = self.vertices[0].x, self.vertices[0].y, self.vertices[0].z
...
class StandardModels:
    def __init__(self):
        ...
        ## ------ test ------ ##
        cube = Model(material = Material((255,0,0)))
        v8 = Vertex(1.2,1.0,1.0)
        v7 = Vertex(1.2,0.0,1.0)
        v2 = Vertex(1.2,0.0,0.0)
        v3 = Vertex(1.2,1.0,0.0)
        s4 = Surface(v8,v7,v2)
        s4.addVertex(v3)
        cube.addSurface(s4)
        self.models['test'] = cube
        ## ------------------ ##
...
class Grid:
    ...
    def display(self, screen):
        #drawing grid
        pygame.draw.aaline(screen, self.xaxis_color, topixel(self.xaxis.start.vx, self.xaxis.start.vy), topixel(self.xaxis.end.vx, self.xaxis.end.vy))
        pygame.draw.aaline(screen, self.zaxis_color, topixel(self.zaxis.start.vx, self.zaxis.start.vy), topixel(self.zaxis.end.vx, self.zaxis.end.vy))

        for edge in self.edges:
            pygame.draw.aaline(screen, self.grid_color, topixel(edge.start.vx, edge.start.vy), topixel(edge.end.vx, edge.end.vy))


class SurfaceSelection:
    def __init__(self, selectable_color = (150,150,150), selected_color = (200,200,200), multiselect_color = (175,175,100)):
    ...
    def display(self, screen):
        ...
        if self.multiselect:
            for surface in self.selected_surfaces:
                ...
                for v in surface.vertices:
                    surface_points.append(topixel(v.vx,v.vy))
                pygame.draw.polygon(screen, self.multiselect_color, surface_points)
