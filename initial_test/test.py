...
# 50 pixel command box
screen_yc = screen_y + 50
...
pygame.init()
screen = pygame.display.set_mode((screen_x, screen_yc))
...
for edge in grid.edges:
    v = numpy.array([[edge.start.x, edge.end.x], [edge.start.y, edge.end.y],
                     [edge.start.z, edge.end.z], [1, 1]])
    v = mainCamera.W2Vm.dot(v)
    # perspective viewing
...
v = numpy.array([[
    grid.xaxis.start.x, grid.xaxis.end.x, grid.zaxis.start.x, grid.zaxis.end.x
], [
...
while True:
    ...
    for event in pygame.event.get():
        ...
        #drawing grid
        pygame.draw.aaline(screen, grid.xaxis_color,
                           topixel(grid.xaxis.start.x, grid.xaxis.start.y),
                           topixel(grid.xaxis.end.x, grid.xaxis.end.y))
        pygame.draw.aaline(screen, grid.zaxis_color,
                           topixel(grid.zaxis.start.x, grid.zaxis.start.y),
                           topixel(grid.zaxis.end.x, grid.zaxis.end.y))
        ...
        for edge in grid.edges:
            ...
            pygame.draw.aaline(screen, grid.grid_color,
