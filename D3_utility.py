...
##        self.x, self.y, self.z = v[0][0], v[1][0], v[2][0]
    ...
    def back_face_detection(self):
        look_vector = (self.lookatpos[0] - self.x, self.lookatpos[1] - self.y, self.lookatpos[2] - self.z)
        for model in self.models:
            if isinstance(model, Model):
                for surface in model.surfaces:
                    look_dot_surface = surface.a * look_vector[0] + surface.b * look_vector[1] + surface.c * look_vector[2]
                    if look_dot_surface > 0:
                        surface.backface = True
                    else:
                        surface.backface = False

    def z_buffer_rendering(self,screen, window_min_x, window_max_x, window_min_y, window_max_y, onePixelInNormal, scanline_min_x = -100, scanline_max_x = 100, max_depth = -100):
        ...
        window_max_pix_x,  window_max_pix_y = topixel(window_max_x, window_max_y)
        window_min_pix_x,  window_min_pix_y = topixel(window_min_x, window_min_y)
        look_vector = (self.lookatpos[0] - self.x, self.lookatpos[1] - self.y, self.lookatpos[2] - self.z)
                ...
                if model.shading == Shading.FLAT:
                    for surface in model.surfaces: #flat shading
                        ...
                        if surface.backface:
                            ...
                            continue
                        c_vec = numpy.array((look_vector[0] + surface.a, look_vector[1] + surface.b, look_vector[2] + surface.c))
                        a = c_vec.dot(self.u)
                        b = c_vec.dot(self.v)
                        c = c_vec.dot(self.n)
                        ...
                        d = -(a*svx + b*svy + c*svz)
                        lvec = [self.lights[0].x - surface.center.x , self.lights[0].y - surface.center.y, self.lights[0].z - surface.center.z]
                        ldotn = (lvec[0] * surface.a + lvec[1] * surface.b + lvec[2] * surface.c)
                        norm_ldotn = math.sqrt(lvec[0]*lvec[0] + lvec[1]*lvec[1] + lvec[2]*lvec[2])
                        ldotn = ldotn / norm_ldotn
                        diff_intensity = max((ldotn * model.material.kd), 0)
                        
                        vvec = [self.x - surface.center.x , self.y - surface.center.y, self.z - surface.center.z]
                        h = (lvec[0] + vvec[0], lvec[1] + vvec[1], lvec[2] + vvec[2])
                        hdotn = (h[0] * surface.a + h[1] * surface.b + h[2] * surface.c)
                        norm_hdotn = math.sqrt(h[0]*h[0] + h[1]*h[1] + h[2]*h[2])
                        hdotn = hdotn / norm_hdotn
                        spec_intensity = max(model.material.ks * (hdotn ** model.material.ns), 0)

                        dist = lvec[0] * lvec[0] + lvec[1] * lvec[1] + lvec[2] * lvec[2]
                        
                        intensity = Light.ia * model.material.kd + self.lights[0].i * (diff_intensity + spec_intensity) / (Light.a0 + Light.a2 *dist)
                        _color = (int(min(model.material.color[0] * intensity, Light.mpv)), int(min(model.material.color[1] * intensity, Light.mpv)), int(min(model.material.color[2] * intensity,Light.mpv))) #flatshading
                        #print(_color)
                        ...
                        while cur_y >= max(surface.min_vy, window_min_y):
                            ...
                            for edge in surface.edges:
                                ...
                                if (lineIntersects(_edge, scanline)):
                                    ...
                                    intersect_x = (cur_y - edge.end.vy) * edge.m_inv + edge.end.vx
                            ...
                            if len(intersected_points) % 2 != 0:
                                print("Before: Odd number of intersections in z-buffer")
                            ...
                            if min_index > (n - max_index):
                                intersected_points = [window_min_x,  window_max_x]
                            else:
                                if min_index > 0:
                                    del intersected_points[0 : min_index ]
                                new_n = len(intersected_points) - 1
                                if (new_n - max_index + 1) <= new_n :
                                    del intersected_points[new_n - max_index + 1 : new_n + 1]
                            ...
                            if len(intersected_points) % 2 != 0:
                                print("After: Odd number of intersections in z-buffer")
                                print(intersected_points)
                                print(model.surfaces.index(surface))
                                return
                            ...
                            while i < (len(intersected_points) - 1):
                                #initial 1/z value
                                z_inv = (a * intersected_points[i] + b * cur_y + c * self.Zvp)/ (-d * self.Zvp)
                                ...
                                while cur_pix_x <= next_pix_x:
                                    ...
                                    if z_inv < z_buffer[z_index]:
                                        ...
                                        screen.set_at((cur_pix_x, cur_pix_y),_color)
                                    z_inv += k
                                    cur_pix_x += 1
                    ...
                    for vertex in model.vertices: #setting vertex intensities
                        lvec = [self.lights[0].x - vertex.x , self.lights[0].y - vertex.y, self.lights[0].z - vertex.z]
                        ldotn = (lvec[0] * vertex.normal[0] + lvec[1] * vertex.normal[1] + lvec[2] * vertex.normal[2])
                        norm_ldotn = math.sqrt(lvec[0]*lvec[0] + lvec[1]*lvec[1] + lvec[2]*lvec[2])
                        ldotn = ldotn / norm_ldotn
                        diff_intensity = max((ldotn * model.material.kd), 0)
                        
                        vvec = [self.x - vertex.x , self.y - vertex.y, self.z - vertex.z]
                        h = (lvec[0] + vvec[0], lvec[1] + vvec[1], lvec[2] + vvec[2])
                        hdotn = (h[0] * vertex.normal[0] + h[1] * vertex.normal[1] + h[2] * vertex.normal[2])
                        norm_hdotn = math.sqrt(h[0]*h[0] + h[1]*h[1] + h[2]*h[2])
                        hdotn = hdotn / norm_hdotn
                        spec_intensity = max(model.material.ks * (hdotn ** model.material.ns), 0)
                        ...
                        vertex.intensity = Light.ia * model.material.kd + self.lights[0].i * (diff_intensity + spec_intensity) / (Light.a0 + Light.a2 *dist)
                        
                    for surface in model.surfaces: #gouraud shading
                        if surface.backface:
                            continue
                        c_vec = numpy.array((look_vector[0] + surface.a, look_vector[1] + surface.b, look_vector[2] + surface.c))
                        a = c_vec.dot(self.u) #calculating a,b,c,d in terms of viewing coordinate
                        b = c_vec.dot(self.v)
                        c = c_vec.dot(self.n)
                        svz = surface.vertices[0].vz  
                        svx = surface.vertices[0].vx * svz / self.Zvp
                        ...
                        while cur_y >= max(surface.min_vy, window_min_y):
                            ...
                            while i < (len(intersected_points) - 1):
                                ...
                                cur_pix_x = topixelx(cur_x)
                                #initial 1/z value
                                z_inv = (a * cur_x + b * cur_y + c * self.Zvp)/ (-d * self.Zvp)
                                ...
                                i3 = intersected_points[i+1][1].start.intensity
                                y3 = intersected_points[i+1][1].start.vy
                                i4 = intersected_points[i+1][1].end.intensity
                                y4 = intersected_points[i+1][1].end.vy
                                i5 = (cur_y - y2)*i1/(y1 - y2) + (y1 - cur_y)*i2/(y1 - y2)
                                i6 = (cur_y - y4)*i3/(y3 - y4) + (y3 - cur_y)*i4/(y3 - y4)
                                if cur_x != next_x:
                                    ik = (i6 - i5)*onePixelInNormal / (next_x - cur_x)
                                else:
                                    ik = 0.0
                                _intensity = i5
                                while cur_pix_x <= next_pix_x:
                                    ...
                                    if z_inv < z_buffer[z_index]:
                                        z_buffer[z_index] = z_inv
                                        _color = (int(min(model.material.color[0] * _intensity, Light.mpv)), int(min(model.material.color[1] * _intensity, Light.mpv)), int(min(model.material.color[2] * _intensity,Light.mpv)))
                                        screen.set_at((cur_pix_x, cur_pix_y),_color)
                                    _intensity += ik
                                    z_inv += k
                                    cur_pix_x += 1
